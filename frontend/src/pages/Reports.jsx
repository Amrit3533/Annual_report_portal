...
export default function Reports() {
  ...
  // Show toast if redirected from CreateReport
  useEffect(() => {
    if (location.state && location.state.toast) {
      setToast({ type: "success", message: location.state.toast });
      // Remove toast from history state so it doesn't show again on back/refresh
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .new-btn:hover { background: var(--accent-dk); color: #fff; transform: translateY(-1px); box-shadow: 0 2px 8px 0 rgba(200,82,42,0.10); }
